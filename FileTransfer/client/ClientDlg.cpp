...
{	
	ShellExecute(NULL, "open", "D:\\", NULL, NULL, SW_SHOWNORMAL);
}
...
void CClientDlg::OnBtnClickedSend()
{
    ...
    sockRecv.Receive((ULONGLONG*)&fileSize, sizeof(fileSize));
    myFile.Seek(fileSize, CFile::begin);

    ULONGLONG dwRead = fileSize;
	while (dwRead < StreamFileInfo.nFileSizeLow)
	{
		byte* data = new byte[CHUNK_SIZE];
		UINT dw = myFile.Read(data, CHUNK_SIZE);
