...
void CServerDlg::recvFile()
{
    ...
    if (PathFileExists(strFileName))
    {
        ...
        fileSize = tempFile.GetLength();
        tempFile.Close();
        sockClient.Send((ULONGLONG*)&fileSize, sizeof(fileSize));

        destFile.Open(strFileName, CFile::modeNoTruncate | CFile::modeWrite | CFile::typeBinary);
        destFile.SeekToEnd();       
    }
    else
    {
        destFile.Open(strFileName, CFile::modeCreate | CFile::modeWrite | CFile::typeBinary);
        sockClient.Send((ULONGLONG*)&fileSize, sizeof(fileSize));
    }

    ULONGLONG dwRead = fileSize;
...
void CServerDlg::OnBtnClickedStart()
{
	...
	if (SOCKET_ERROR == m_sListenSocket.Listen())
	{
		CString m_ErrorMsg;
		m_ErrorMsg = GetErrorMsg();
		MessageBox(m_ErrorMsg);
