...
/*-------------参数初始化------------*/	
	...
	SDL_Window *window = SDL_CreateWindow("Player", 100, 100, 0, 0, SDL_WINDOW_RESIZABLE);
	if(!window)
	{
    ...
    SDL_Texture *texture = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_YV12, SDL_TEXTUREACCESS_TARGET, vCodecCtx->width, vCodecCtx->height);
		...
		if (packet.stream_index == videoStream) {//检查视频媒体流类型标识
			...
			* convert the packet to a frame for us and set frameFinisned for us when we have the next frame
			...
			if (frameFinished) {
				...
				sws_scale(sws_ctx, (uint8_t const * const *)pFrame->data, pFrame->linesize, 0, vCodecCtx->height, pFrame->data, pFrame->linesize);
